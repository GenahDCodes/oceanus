...
export default function ShipmentProgressBar({ status }) {
  ...
  return (
    <div className="bg-white border border-steel-200 rounded p-5">
        ...
        {steps.map((step, index) => {
          ...
          return (
            <div key={step} className="flex items-center gap-2">
              <div className={`w-3 h-3 rounded-full ${active ? 'bg-ocean-500' : 'bg-steel-200'}`} />
              <p className={`text-[11px] uppercase tracking-wide ${active ? 'text-navy-900 font-semibold' : 'text-steel-400'}`}>
                {step}
              </p>
